    ...
    IVesting
{
    ...
    function getVestingInforById(uint256 _vestingId) public override view returns(
        ...
        periodTime = vestingInfo.periodTime;
        numberClaim = vestingInfo.numberClaim;
    }
    ...
    function _getAmountCanClaim(uint256 _vestingId) internal view returns(uint256) {
        ...
        uint256 withdrawable = 0;
        uint256 durationTime = vestingInfo.numberClaim*vestingInfo.periodTime;
        uint256 endTime = vestingInfo.startTime.add(durationTime);
        if (block.timestamp < endTime && block.timestamp > vestingInfo.startTime && vestingInfo.status == 1) {
            ...
            uint256 vestedSlicePeriods = timeFromStart/vestingInfo.periodTime;
            uint256 vestedSeconds = vestedSlicePeriods*vestingInfo.periodTime;
            withdrawable = (vestingInfo.totalAmount * vestedSeconds/durationTime);
